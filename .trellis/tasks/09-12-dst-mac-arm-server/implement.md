...
- [x] A1（fd9453d，基点=upstream/main d189b9e，完整历史保留；注意：1.6.1 tag 是重构前旧布局，不可作基点）以 tag 1.6.1 对应 commit 为基点 clone 上游到本仓库（保留 git 历史），建 `upstream` remote，`main` 分支为开发基线
      验证：`git log` 含上游历史；`go build ./...` 通过（amd64 本机跑通即可）
- [x] A2 GPL-3.0 合规处理：保留 LICENSE；README 顶部标注 fork 来源（carrot-hu23/dst-admin-go）与修改说明（占位，Phase E 完善）
      验证：LICENSE 与版权声明完整

## Phase B：Go 源码改造（design.md §2）

- [x] B0（DD macos-arm64 实测通过；发现游戏自带 steamclient.so、mod 主流为 CDN zip 型需解压兜底，详见 research §7）本机验证 DepotDownloader（开发红利，R2 文档 §6）：下载 `DepotDownloader-macos-arm64.zip`，手动跑
...
- [x] B1 新增 `internal/service/steam/depotdownloader.go`：路径解析（env > cluster.Steamcmd > /opt 默认）+ 参数数组 exec + 日志透传 + 单测
      验证：`go test ./internal/service/steam/` 通过；命令构造单测覆盖 beta/非 beta
- [x] B2 替换游戏安装/更新：`update/linux_update.go` 改调 `DownloadApp`（Beta==1 → `<dir>-beta` + `-branch updatebeta`）；Windows 分支不动
      验证：单测 + `go build`；`go vet ./...`
- [x] B3 替换模组下载：`mod_service.go getModInfoConfig` 改调 `DownloadPubfile`，`dir=<Mod_download_path>/steamapps/workshop/content/322330/<modId>`；保留"已存在跳过"；失败语义与现有一致（返回空 map + 日志）
      验证：单测覆盖路径拼接契约
- [x] B4 交叉编译冒烟：`CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -o /tmp/dst-mac-server cmd/server/main.go`
      验证：产出 arm64 ELF（`file` 核对）

## Phase C：Docker 镜像（design.md §3）

- [x] C1 编写 `Dockerfile`（多阶段：builder 交叉编译 + 提取上游 dist；runtime 装 dotnet8/box64/DD/steamcmd 解包/amd64 运行库）
      验证：`docker buildx build --platform linux/arm64 -t dst-mac-server:dev .` 本地构建成功
- [x] C2 编写 `scripts/docker-arm64/docker-entrypoint.sh`（播种 data、steamclient.so、appmanifest 条件首装、DST_FORCE_UPDATE、exec 面板）+ 播种用 `docker_dst_config`（Bin=2664）
      验证：`docker run --rm` 反复启动：首启下载、二启秒过（日志核对）
- [x] C3 容器内冒烟：`ldd` bin64 无 not found；容器内手动触发面板安装游戏成功
      验证：安装日志为 DD 输出；`appmanifest_343050.acf` 生成
- [x] C4（compose 示例完成）`docker-compose.yml` 示例（端口 8082/10888-10999 udp、卷 /app/data 与游戏目录）
